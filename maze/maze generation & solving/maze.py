...
def affichage (L):
    ...
    for i in range (len(L)):
        ...
        for j in range (len(L[0])) :
            ...
            if L[i][j]==1:
                p.draw.rect(fenetre, colorMur, (50+caseSize*j, 50+caseSize*i, caseSize, caseSize))
            if L[i][j]==3:
                p.draw.rect(fenetre, 'red', (50+caseSize*j, 50+caseSize*i, caseSize, caseSize))
            if L[i][j]==4:
                p.draw.rect(fenetre, 'green', (50+caseSize*j, 50+caseSize*i, caseSize, caseSize))
    p.display.flip()
...
def affichageSolution (S):
    ...
    for i in range (len(S)):
        ...
        p.draw.rect(fenetre, gradient[i], (50+caseSize*P[1], 50+caseSize*P[0], caseSize, caseSize))
        # time.sleep(0.02)
        p.display.flip()

#- Affichage d'un chemin

def affichageChemin (S):
    for i in range (len(S)):
        P=S[i]
        p.draw.rect(fenetre, "yellow", (50+caseSize*P[1], 50+caseSize*P[0], caseSize, caseSize))
        p.display.flip()
...
def labyAlea_optimise(n, m):
    ...
    labyrinthe[y-1][2]=0 """
    labyrinthe[z][m-1]=4
    """     labyrinthe[z-1][m-2]=0
    labyrinthe[z+1][m-2]=0
    labyrinthe[z][m-2]=0
    labyrinthe[z-1][m-3]=0
    labyrinthe[z+1][m-3]=0
    labyrinthe[z][m-3]=0 """
    return labyrinthe
...
font1 = p.font.SysFont('sysfont', 50)
font2 = p.font.SysFont('sysfont', 1)
caseSize=2
# Liste_à_étudier = labyAlea_ralenti(200,200)





Liste_à_étudier = labyAlea_optimise(400,500)
...
poss = labyPoss_optimized(Liste_à_étudier)
